...
\RetVal{Zero if \cvar{name} is permissible name, a non-zero value otherwise.}
...
\Desc{Create a new subkey \cvar{name} in the key node \cvar{dir} with type 
  \ctext{affNodeVoid} (no associated data type). The type may be changed later. The function calls
...
{\Syn struct~AffTree\_s~*aff\_tree\_init (struct~AffSTable\_s~*stable,~int~size);\par}
\Desc{Allocate and initialize an AFF tree structure with only one node,
  which is root. The name of the root is an empty string ``''. The initial
size of the tree is supplied by \ctext{size} which will be adjusted if
unreasonable. The user should provide \ctext{stable} to keep associated key data in.}
...
{\Syn struct~AffSTable\_s~*aff\_stable\_init (int size);\par}
\Desc{Allocate and initialize an empty symbol table. Suggested initial table
size is \ctext{size}. The library will adjust the size if unreasonable.}
